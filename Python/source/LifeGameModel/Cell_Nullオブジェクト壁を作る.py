...
class CellArray(object):
	...
	# セルの集合を初期化.
	# 一番外側は境界としてNullオブジェクトとする.(Nullオブジェクトは操作をしても何も作用をしない)
	def __init__(self, i_Col, i_Row):
		...
		for theRow in range(self._row_max):
			...
			for theCol in range(self._col_max):
				if( (theRow==0) or (theRow==self._row_max-1)): theElement = Null()
				elif( (theCol==0) or (theCol==self._col_max-1) ): theElement = Null()
				else: theElement = Cell()
...
#		]
		...
		for row in range(self._row_max):
			for col in range(self._col_max):
				...
				if(isinstance(target_cell, Cell)): 
					# Nullオブジェクトで囲まれている事を想定している為,境界を気にする必要は無い.
	...
	def get_state(self, col, row):
		_cell = self._cell_array[row][col]
		_my_state = _cell.current_state
		return _my_state


	# セルの状態を設定.
	def set_state(self, col, row, state):
		self._cell_array[row][col].current_state = state
		self._cell_array[row][col].send_state()

	# セルを取得.
	def get_cell(self, col, row):
		return self._cell_array[row][col]
